...
//Next, download and unzip "AKWF_Mozzified" folder. Copy and Paste ALL the <AKWF_....512.h> waveform files found within this folder into the "tables" folder located in Arduino/libraries/Mozzi/tables. 
//Do Not Simply put the unzipped "AKWF_Mozzified" folder into the "tables" folder. You must select and copy all the files inside "AKWF_Mozzified" folder and paste them all into the "tables" folder.
...
Oscil <AKWF_oscchip_0100_512_NUM_CELLS, AUDIO_RATE> aSin3;
...
void setup(){
  ...
  randomSeed(mozziAnalogRead(A7));     //takes a quick reading of potentiometer (knob) A7 and bases its psuedo random # generator on it to calculate the following random values
...
// Serial.begin(9600);   //for debugging 
 ...
 pinMode(12,INPUT_PULLUP);  //button 1     //4 key buttons are "pulled-up" to HIGH +5V signal. When pressed, they connect the input pin to GROUND which will now 
 pinMode(11,INPUT_PULLUP);  //button 2      read as LOW in the code. this may seem confusing at first but HIGH means that button IS NOT pressed and 
 pinMode(6,INPUT_PULLUP);   //button 3      LOW means that the button IS pressed
 pinMode(10,INPUT_PULLUP);  //button 4


 pinMode(0,INPUT_PULLUP);    //waveform        //          
 pinMode(2,INPUT_PULLUP);    //voicing
 pinMode(5,INPUT_PULLUP);    //play style
 pinMode(8,INPUT_PULLUP);    //speed
 pinMode(7,INPUT_PULLUP);    //rhythm



 pinMode(13,OUTPUT);    //6
 pinMode(16,OUTPUT);    //5       //Option Outputs 1-6
 pinMode(17,OUTPUT);    //4
 pinMode(18,OUTPUT);    //3
 pinMode(19,OUTPUT);    //2
 pinMode(1,OUTPUT);     //1
...
void updateControl(){
 ...
 phold1=smoothKnob1.next(mozziAnalogRead(A7));    // from right to left:  takes raw analog knob reading (mozziAnalogRead()) ,smooths them (smoothKnob), then sets  
 phold2=smoothKnob2.next(mozziAnalogRead(A6));     phold variable (final pitch) with the final smoothed output
 phold3=smoothKnob3.next(mozziAnalogRead(A0));
 phold4=smoothKnob4.next(mozziAnalogRead(A1));
...
if(outPut2!=0){                                                  //if voicing is zero than all gains are on / gains 1-8 are either 1 (on) or 0 (off)
  ...
  gain8=1;
    ...
    if(digitalRead(12)==LOW){            ///INCREASE BOUNCE TIMES WITH KEYS
...
///////////////////////////////////////////////////////////////  INPUT JACKS 
  ...
  if(digitalRead(0)==HIGH&&reset1){               // here is input jack 1. whenever it reads HIGH, it resets itself and waits to read LOW 
      reset1=false;
  ...
  if(digitalRead(0)==LOW){                 //here the input jack reads LOW and continually increments its pulseCounter variable until it reads HIGH again.  
...
///////////////////////////////////////////////////////////////////////////////
    ...
    if(digitalRead(2)==HIGH&&reset2){               //VOICING INPUT JACK. Each input jack functions the same here.  Incrementing its pulseCounter variable as long as
...
///////////////////////////////////////////////////////////////////////////////////
      ...
      if(digitalRead(5)==HIGH&&reset3){               //outPut3     MODE SELECT
      reset3=false;
...
if(outPut3==7){
  ...
  ///////////////////////////////////////////////////////////////////////////////////
      ...
      if(digitalRead(8)==HIGH&&reset4){               //outPut4     SPEED
      reset4=false;
...
if(mode==2||mode==3){               //If in Seq mode (patterns) or in FM mode (freqmodz)               
  ...
  if(digitalRead(12)==LOW){                        //Keys changing values of the pitchAlter variable which is added to the final pitch output  
...
envGain8=tempGain;
  ...
  if(speedsTimer.ready()){             //in mozzi, things like delay() and delayMillis() do not work. You must learn to use the EventDelays instead to perform this 
      ...
      if(digitalRead(11)==HIGH){     //if the HOLD SEQUENCE key button is NOT pressed (HIGH)
        if(digitalRead(6)==LOW){    //if the random step button is pressed then choose the next seq step randomly between 1-4
          seqPos=random(0,5);        
        } else {             
...
envGain8=255;
  ...
  ///////////////////////////////////////////////////////////////////////////////////
      ...
      if(digitalRead(7)==HIGH&&reset5){               //outPut5     RHYTHMS
      reset5=false;
